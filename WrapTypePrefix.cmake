...
WRAP_TYPE("RGBPixel" "RGB")
  WRAP_TYPES("${ITKM_UC}" "${ITKT_UC}" "RGBUC")
  WRAP_TYPES("${ITKM_US}" "${ITKT_US}" "RGBUS")
END_WRAP_TYPE()
SET(itk_Wrap_RGBPixel ${WRAPPER_TEMPLATES})

WRAP_TYPE("Image" "I")
  FOREACH(d ${WRAP_ITK_DIMS})
    WRAP_TYPES("${ITKM_F}${d}"  "${ITKT_F},${d}"  "F")
    WRAP_TYPES("${ITKM_D}${d}"  "${ITKT_D},${d}"  "") # needed for BSplineDeformableTransform
    WRAP_TYPES("${ITKM_UC}${d}" "${ITKT_UC},${d}" "") # needed to save in 8 bits
    WRAP_TYPES("${ITKM_US}${d}" "${ITKT_US},${d}" "US")
    WRAP_TYPES("${ITKM_UL}${d}" "${ITKT_UL},${d}" "") # needed for watershed and relabel filters
    WRAP_TYPES("${ITKM_SC}${d}" "${ITKT_SC},${d}" "SC")
    WRAP_TYPES("${ITKM_SS}${d}" "${ITKT_SS},${d}" "SS")
    WRAP_TYPES("${ITKM_SL}${d}" "${ITKT_SL},${d}" "SL")
    WRAP_TYPES("${ITKM_VF${d}}${d}" "${ITKT_VF${d}},${d}" "VF")
    WRAP_TYPES("${ITKM_VD${d}}${d}" "${ITKT_VD${d}},${d}" "VD")
    WRAP_TYPES("${ITKM_CVF${d}}${d}" "${ITKT_CVF${d}},${d}" "CVF")
    WRAP_TYPES("${ITKM_CVD${d}}${d}" "${ITKT_CVD${d}},${d}" "CVD")
    WRAP("${ITKM_O${d}}${d}" "${ITKT_O${d}},${d}")
    WRAP_TYPES("${ITKM_RGBUC}${d}" "${ITKT_RGBUC},${d}" "RGBUC")
    WRAP_TYPES("${ITKM_RGBUS}${d}" "${ITKT_RGBUS},${d}" "RGBUS")
  ENDFOREACH(d)
END_WRAP_TYPE()
...
WRAP_TYPE("BinaryBallStructuringElement" "SE")
  FOREACH(d ${WRAP_ITK_DIMS})
    WRAP_TYPES("${ITKM_F}${d}"  "${ITKT_F},${d}"  "F")
    WRAP_TYPES("${ITKM_D}${d}"  "${ITKT_D},${d}"  "D")
    WRAP_TYPES("${ITKM_UC}${d}" "${ITKT_UC},${d}" "UC")
    WRAP_TYPES("${ITKM_US}${d}" "${ITKT_US},${d}" "US")
    WRAP_TYPES("${ITKM_UL}${d}" "${ITKT_UL},${d}" "UL")
    WRAP_TYPES("${ITKM_SC}${d}" "${ITKT_SC},${d}" "SC")
    WRAP_TYPES("${ITKM_SS}${d}" "${ITKT_SS},${d}" "SS")
    WRAP_TYPES("${ITKM_SL}${d}" "${ITKT_SL},${d}" "SL")
  ENDFOREACH(d)
