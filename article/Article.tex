...
WrapITK provides an interface to some C++ compiled code, and so, execution time
...
the required code, WrapITK binary contains all the classes wrapped, and so
WrapITK can take a significant amount of memory.

Some comvenient features, like sequence management in python, can be quite
...
from the console, and so show the interpreter prompt:
...
In the example above, \verb$12+3$ is what is wrote in the interpreter, and
\verb$15$ is the result. \verb$2>$, \verb$3>$, \verb$4>$ are the prompt
of the interpreter.
...
\end{verbatim}
  ...
  \item \verb$itk.echo(object, file)$ is a convenient function to call the \verb$Print()$
...
method do a very similar job with a better integration with python.
...
passed as parameter, and the filter passed as parameter will be added to the filter
list.
...
this method, or subclass pipeline to implement another \verb$GetInput()$ method.

  \item \verb$Update( self )$ update the pipeline by calling \verb$Update()$ method
on the last filter in the pipeline.

  \item \verb$__getitem__( self, i )$ and \verb$__len__( self )$ provide common
python list manipulation interface to the pipeline object.

\end{itemize}

Example:
ITK current implementation of morphological dilation, erosion, opening and closing
can be very inefficient with large structuring elements. Also, WrapITK only give
access to ball structuring element. The following class illustrate the use of
\verb$itk.pipeline$ class to implement a efficient opening in 3 dimensions with
a box structuring element. We here take advantage of structuring element decomposition:
...
The \verb$itk.pipeline$ encapsulate the 6 filters needed to perform the efficient
opening, and take care of setting the structuring element for all the internal
...
This project is a python only project. It requires to have Numeric, Numarray or numpy installed
...
\verb$GetImageFromArray()$ method to convert respectively a array to an ITK image, and an
ITK image to an array. There is no need to instantiate a \verb$PyBuffer$ object: the methods
are \verb$static$.
...
This project provide new feature for all the languages, including C++.
Its directory structure reflect that. 
...
prevent overriding the reference files during the test, if the build
...
We thank Andr\'e Bongers for his help to debug java build on windows plateform.
