...
and CableSwig \cite{CableSwigWebSite}. This project aims to be
...
For the user which don't want to use darcs \cite{DarcsWebSite} but want the last development version,
...
\verb$itk.show3D()$ requires to have ItkVtkGlue\footnote{ItkVtkGlue can be found in
the {\em ExternalProject} directory of WrapITK}, PyQt \cite{PyQtWebSite}, and iPython
...
parameter which controls how many image template parameters are created (see
above). The optional third parameter is a dimensionality condition (see above
also).
